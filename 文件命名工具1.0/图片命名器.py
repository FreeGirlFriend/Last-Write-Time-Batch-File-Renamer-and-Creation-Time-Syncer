...
class FileRenamerApp:
    ...
    def _build_ui(self):
        ...
        self.ext_entry = tb.Entry(
            action_bar, textvariable=self.ext_filter, width=36,
        )
        self.ext_entry.pack(side=LEFT, padx=(6, 4))
        hint = tb.Label(
            action_bar,
            text="* = 全部  |  多个用空格分隔  |  例: .jpg .png .mp4 .pdf",
        ...
        # ---- 底部：进度 + 日志 ----
        bottom = tb.Frame(self.root, padding=(16, 2, 16, 10))
        bottom.pack(fill=X)
        ...
        self.log_text = tb.Text(
            log_container, height=6, wrap="word",
